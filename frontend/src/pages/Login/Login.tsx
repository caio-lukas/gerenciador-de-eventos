...
export default function Login({ navigation }: Props) {
  ...
  const handleLogin = async () => {
    ...
    try {
      // Ajuste o endpoint '/auth/login' se o seu backend utilizar outro caminho (ex: /login)
      const response = await api.post('/auth/login', {
        email,
        senha
      });

      // Extrai o token da resposta (ajuste a propriedade 'token' conforme o DTO do Spring Boot)
      ...
      const mensagemErro = error.response?.data?.mensagem || 'E-mail ou senha inválidos.';
